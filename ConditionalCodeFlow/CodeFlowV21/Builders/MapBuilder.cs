    ...
    public class MapBuilder
    {
        ...
        public void AddNode(Node node, string name = "")
        {
            if (!string.IsNullOrEmpty(name))
                node.Name = name;
            _map.nodes[node.nodeId] = node;
        }

        public void AddConnection(int startNodeId, int endNodeId, Connection connection = null)
        {
            var startNode = _map.nodes[startNodeId];
            var endNode = _map.nodes[endNodeId];
